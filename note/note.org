...
- 私有 page cache: 进程加载的动态链接库, 使用 copy on write, 需要修改时才复制出新的内存
- 共享 page cache: 不同进程读写同一文件
- 私有 anonymous pages: 进程的 heap, stack
- 共享 anonymous pages: 共享内存
...
mm 为空的即为内核线程, 内核线程的 active_mm 是借用用户线程的
...
进程状态:
R Running 正在运行或者在 running queue 中
D Disk Sleep 正在不可中断睡眠中
S Interruptible Sleep 正在可中断睡眠中
Z Zombie 僵尸进程
I Idle 空闲状态, 没有负载的不可中断睡眠的内核线程
...
* c/cpp
link: -fvisibility=hidden 或 --version-script 控制库中符号的可见性

多态类的 typeinfo 由其 vtable 指向

enable_shared_from_this 使用 CRTP 实现 shared_from_this, 使得使用 this 指针时正确地增加 use_count

clz, ctz, popcount
clz 前导0个数, ctz 后置0个数, popcount 1的个数
gcc 中使用 __builtin_clz, __builtin_ctz, __builtin_popcount

placement new 利用已分配好的内存, 并调用构造函数
#+begin_src c++
  void* operator new(std::size_t, void*);
#+end_src

编译器会给每一个包含虚函数或继承了虚函数的类自动建立一个虚函数表
...
进程用时分为实际时间(Real) 用户 CPU 时间(User) 系统 CPU 时间(Sys)
User 和 Sys 通过wait (2) 或 times (2) Real 通过 gettimeofday (2)
times 得到的 CPU 时间包括所有线程和调用了 wait 的进程

rusage 结构体中也存了 User 和 Sys, 通过 getrusage (2) 或 wait4 (2) 获取
#+begin_src c
  struct rusage {
...
clock_gettime 可以指定线程或进程, 获取挂钟时间
...
- 350法则
  要实现类的用户定义拷贝语义需要同时实现 destructor, copy constructor, copy assignment
  要实现类的用户定义移动语义需要同时实现 destructor, copy constructor, copy assignment, move constructor, move assignment
  要使用隐式的这5个函数, 不能定义任一用户定义的其中一种

- 复制消除 copy elision
  非必须不会将纯右值实质化, 省略复制及移动构造函数, 导致零复制的按值传递语义
...
- lvalue, xvalue, prvalue
  ...
  lvalue 和 xvalue 具有多态, prvalue 不具有多态
...
** smart pointer
- unique_ptr
  不可复制构造 (CopyConstructible) 或可复制赋值 (CopyAssignable)
  可移动构造 (MoveConstructible) 和可移动赋值 (MoveAssignable)
  需要用 std::move, std::forward, emplace_back
  #+begin_src c++
...
- shared_ptr
  函数调用时传值才会修改 use count

- weak_ptr
  调用 lock() 使得其所指向的 shared ptr 的 use count 增加
...
*** eventpoll 等待队列 wq 和 poll_wait
wq 等待队列保存对这个 eventpoll 调用 epoll_wait() 的所有进程

poll_wait 等待队列处理递归唤醒:
epollfd1 监视了 2 个非 epoll类型的 fd
epollfd2 监视了 epollfd1 和 2 个非 epoll类型的 fd
如果 epollfd1 所监视的 2 个 fd 中有可读事件触发
fd 的 ep_poll_callback 回调函数会触发将 fd 放到 epollfd1 的 rdllist 中
此时 epollfd1 本身的可读事件也会触发
就需要从 epollfd1 的 poll_wait 等待队列中找到 epollfd2
调用 epollfd1 的 ep_poll_callback(将 epollfd1 放到 epollfd2 的 rdllist 中)

*** eventpoll ovflist
ovflist 在 rdllist 被占用时, 用来在不持有 ep->lock 的情况下收集有就绪事件的 fd

*** eventpoll lock 和 mtx
lock:  because we manipulate objects from inside the poll callback, that might be triggered from a wake_up() that in turn might be called from IRQ context. So we can't sleep inside the poll callback and hence we need a spinlock

mtx: 用于 rdllist 或红黑树的访问

*** epitem pwqlist
存放 eppoll_entry, eppoll_entry 去调用 ep_poll_callback()
